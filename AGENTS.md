...
- `grid_config`: template parsing, slot/crossing construction, candidate filtering/ranking, and rendering. `ParsedTemplate::parse` is the only template parser, and `ParsedTemplate::fold_diacritics` is the only normalization a template gets &mdash; never `normalize_word`, which is for dictionary content and would strip the `#`/`.` framing; `OwnedGridConfig` owns per-template setup and borrows the corpus at `to_config_ref(&word_list)`; `GridConfig<'a>` is the borrowed solver view.
...
- `oracle`: persistent fillability oracle. Derives its diacritic policy from the corpus rather than taking a second copy, holds one `WordList` across probes, and answers `Unfillable` (a propagation or exhausted-search proof) / `Fillable` / `Unknown` (budget exhausted). Never collapses the third state into the first. Each probe brackets configuration with `WordList::snapshot`/`rewind`, so the hidden entries a fully specified slot forces into the dictionary stay local to that grid.
...
- `scripts/oracle.py`: `--serve` client, `OraclePool`, and the three-valued `Verdict` whose `__bool__` raises on purpose; `scripts/test_oracle_client.py` covers its row handling, pool concurrency and startup teardown. `scripts/screen_audit.py` measures the arc-consistency screen against real fill attempts.
...
The one Python test lives beside its subject: `python3 scripts/test_oracle_client.py` runs the `scripts/oracle.py` contract tests (row framing, pool concurrency, startup teardown) with no third-party test runner. Only the teardown case spawns a process; the rest use fakes.
